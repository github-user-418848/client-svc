        ...
        if (security::checkCSRF($_POST['csrf_token'])) {
            ...
            if (in_array("MEMBERS UPDATING", $transactions_arr)) {
                if (!isset($_POST["emp_num"])) {
                    header("Location: non-member.php");
                    die();
                }
            ...
            if (!empty($signature)) {
                ...
                $values_arr = array(
                    ":id" => "NEW_EMPLOYER", ":pin" => "NEW_EMPLOYER", ":hdid" => $_SESSION["hdid"], 
                    ":requestor_name" => $_SESSION["requestor_name"], ":priority" => $_SESSION["priority"],
                    ":name" => ($last_name.', '.$first_name.' '.$middle_name), ":addr" => $addr, ":contact" => $contact, ":email" => $email, ":transaction_date" => date("Y-m-d"), ":transaction" => $transactions, 
