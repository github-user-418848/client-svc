<?php
    ...
    if (security::checkMethod("POST")) {
        ...
        $fields = array("transaction", "request", "gov_id", "img-data", "csrf_token", "submit");
        ...
        if (security::checkCSRF($_POST['csrf_token'])) {
            ...
            if ($data && !empty($signature)) {
                ...
                $query = "INSERT INTO tbl_transaction (TRANSACTION_ID, HDID, PIN_PEN, TRANSACTION_DATE, TRANSACTION_DESC, PURPOSE, NAME, ADDRESS, REQUESTOR_NAME, PRIORITY, ID_PRESENTED, QUEUE_NUM, MEMBER_UPDATE_COUNT, CLIENT_TYPE, ESIGNATURE)
                VALUES (:id, :hdid, :pin, :transaction_date, :transaction, :request, :name, :addr, :requestor_name, :priority, :id_presented, :queue_num, :member_update_count, :client_type, :signature)";
                ...
                $values_arr = array(
                    ...
                    ":name" => ($data["LASTNAME"].', '.$data["FIRSTNAME"].' '.$data["MIDDLENAME"]), ":addr" => $data["ADDRESS"], 
                    ":transaction_date" => date("Y-m-d"), ":transaction" => $transactions, ":request" => $request, ":id_presented" => $id_presented,
                    ":queue_num" => $_SESSION["queue"], ":member_update_count" => "1", ":client_type" => "MEMBER", ":signature" => $filepath
                );
